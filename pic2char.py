...
def head2char(workspace, folder, self, outlist):
    ...
    colorlist = ['#FFFACD', '#F0FFFF', '#BFEFFF', '#b7facd',
                 '#ffe7cc', '#fbccff', '#d1ffb8', '#febec0', '#E0EEE0']
    # index用来改变不同字的背景颜色
    index = 0
        ...
        for i in range(16*16):
            ...
            if item[i] == "1":
                ...
                try:
                    ...
                    img = img.resize((eachSize, eachSize),
                                     Image.ANTIALIAS)  # 缩小图片
                    canvas.paste(img, ((i % 16) * 2 * eachSize,
                                       (i // 16) * 2 * eachSize))  # 拼接图片
                # 点阵处右上角图片2/4
                try:
                    ...
                    img = img.resize((eachSize, eachSize),
                                     Image.ANTIALIAS)  # 缩小图片
                    canvas.paste(img, (((i % 16) * 2 + 1) * eachSize,
                                       (i // 16) * 2 * eachSize))  # 拼接图片
                # 点阵处左下角图片3/4
                try:
